...
(function($){
    $.fn.pgwSlideshow = function(options) {
        ...
        var init = function() {
            ...
            pgwSlideshow.window.resize(function() {
                ...
                pgwSlideshow.resizeEvent = setTimeout(function() {
                    ...
                    if (pgwSlideshow.config.maxImgHeight) {
                        if (maxHeight > pgwSlideshow.config.maxImgHeight) {
                            maxHeight = pgwSlideshow.config.maxImgHeight;
                        }
                    }
                    if (pgwSlideshow.config.minImgHeight) {
                        if (maxHeight < pgwSlideshow.config.minImgHeight) {
                            maxHeight = pgwSlideshow.config.minImgHeight;
                        }
                    }
        ...
        var updateHeight = function(height, animate) {
			if(!pgwSlideshow.config.displayCurrents){
				return true;
            }
            ...
            if (typeof pgwSlideshow.plugin.find('.ps-current').animate == 'function') {
                ...
                pgwSlideshow.plugin.find('.ps-current').css('height', height+40);
        ...
        var setup = function() {
            ...
            pgwSlideshow.plugin.find('.ps-list > ul > li').each(function() {
                ...
                if (element.image) {
                    currentElement.html('<img src="' + element.image + '" class="' + element.class + '" alt="' + (element.title ? element.title : '') + '">');
                } else if (element.thumbnail) {
                    currentElement.html('<img src="' + element.thumbnail + '" class="' + element.class + '" alt="' + (element.title ? element.title : '')+ '">');
                }

                if (element.link) {
                    currentElement.html('<a href="' + element.link + '"' + (element.linkTarget ? ' target="' + element.linkTarget + '"' : '') + '>' + currentElement.html() + '</a>');
                }
				currentElement.html('<div>'+currentElement.html()+'</div>');
                pgwSlideshow.plugin.find('.ps-current > ul').append(currentElement);
				if(currentElement.children('div').children('img').attr('class')== 'upload')
				{
					currentElement.children('div').attr('id','upload');
				}
        ...
        var getElement = function(obj) {
            ...
            if ((typeof elementClass != 'undefined') && (elementClass != '') && elementClass != undefined) {
                element.class = elementClass;
                obj.find('img').attr('class', '');
            }
            return element;
        };
        ...
        var finishElement = function(element) {
            ...
            var height = pgwSlideshow.plugin.find('.ps-current .elt_' + element.id + ' img').height();
            if (pgwSlideshow.config.maxImgHeight) {
                if (height > pgwSlideshow.config.maxImgHeight) {
                    height = pgwSlideshow.config.maxImgHeight;
                }
            }
            if (pgwSlideshow.config.minImgHeight) {
                if (height < pgwSlideshow.config.minImgHeight) {
                    height = pgwSlideshow.config.minImgHeight;
                }
            }
            pgwSlideshow.plugin.find('.ps-current .elt_' + element.id + ' img').height(height);
        ...
        var fadeElement = function(element) {
			if(!pgwSlideshow.config.displayCurrents){
				...
				pgwSlideshow.config.slideEventHandler(element.id);
				return;
			}
            ...
            if (pgwSlideshow.currentSlide > 0) {
				...
				currentElement.children('div').removeClass('markcontainer');
				currentElement.children('div').children('img').removeAttr('id','img');
			...
			nextElement.children('div').addClass('markcontainer');
			nextElement.children('div').children('img').attr('id','img');
            ...
            nextElement.css('position', 'absolute').show().animate({
                ...
                finishElement(element);
                pgwSlideshow.config.tabPanelAnimation(img);
            });
		...
		pgwSlideshow.getElement = function(obj)
		{
			return getElement(obj);
		};
		pgwSlideshow.uploadElement = function(element)
		{
			fadeElement(element);
			return true;
		};
